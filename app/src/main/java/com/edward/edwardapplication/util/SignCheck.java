...
public class SignCheck {
     ...
     */
    public static boolean checkSignature(Context context, String packageName, String rightCer) {
        if (StringUtils.isNullOrEmpty(rightCer)) {
            Log.e(TAG, "未给定真实的签名 SHA-1 值");
            return false;
        }

        String cer = getCertificateSHA1Fingerprint(context, packageName).trim();
        rightCer = rightCer.trim();
        if (cer.equals(rightCer)) {
            return true;
        }
        return false;
    }
